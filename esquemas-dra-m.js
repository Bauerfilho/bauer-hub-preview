 ...
 *   <script src="esquemas-dra-m.js"></script>   (antes de </body>)
...
(() => {
  ...
  const LS_KEY = 'ubs2026.v1.m';   // chave única do localStorage (ver "ONDE GRAVA" acima)
  ...
  function renderSecao(sec, topicId) {
    ...
    sec.appendChild(el('h2', 'dm-title', '⭐ Esquemas da Dra. M.'));
  ...
  function exportar() {
    const store = load();
    const payload = {
      _arquivo: 'esquemas-dra-m',
      _versao: 1,
      _aviso: 'Esquemas pessoais e favoritos da Dra. M.. Não contém dados de pacientes.',
    ...
    const a = el('a');
    a.href = url;
    a.download = 'esquemas-dra-m-' + hoje() + '.json';
  ...
  function montar() {
    ...
    sec.id = SECTION_ID;
    sec.hidden = true;
    sec.setAttribute('aria-label', 'Esquemas da Dra. M.');
