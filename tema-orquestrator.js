/* TEMA ORQUESTRATOR — ativador e marcas (screen-only, zero rede, idempotente).
   Autoria: brain, 28/08/2026. API pública: window.TEMA.m(true|false).
   Injeta: selo "ORQUESTRATOR" na appbar + dedicatória à Dra. M. no hero.
...
(function(){
  ...
  function ligarModoDela(){try{if(localStorage.getItem('ubs2026.v1.session'))document.body.classList.add('tema-m')}catch(_){}}
  function boot(){selo();dedicatoria();marcaUnidade();iconesFinos();ligarModoDela();}
  window.TEMA={
    m:function(on){document.body.classList.toggle('tema-m',!!on);}
  };
