/* ============================================================================
   HUBv3 — Módulo "Dra. M. extra" (frentes F-D/E/F/G/H/I/J · contrato de 28/08)
   ----------------------------------------------------------------------------
   ...
   brain em js/farmacia.js e js/dossie-m.js: só consome, não reescreve) ·
...
window.F2X = (() => {
  ...
  /* ==========================================================================
     F-D · LOGIN — overlay de boot. CRM 24644 + senha saude1234.
  ...
  function montarLogin() {
    ...
    fundo.id = 'f2xLogin';
    fundo.innerHTML =
      '<div class="f2x-login-card" role="dialog" aria-modal="true" aria-label="Entrada da Dra. M.">' +
  ...
  /* ==========================================================================
     ...
     Pede o CRM (24644 = a chave) → define senha NOVA 2x → grava em
  ...
  function montarResetSenha() {
    ...
    crm.focus();
    const redefinir = () => {
      if (String(crm.value).trim() !== '24644') { erro.textContent = 'CRM não confere. O reset usa o CRM da unidade.'; return; }
  ...
  // Notificação da troca ao WhatsApp dela (5562996952055):
  ...
  function notificarWhatsAppSenha() {
    ...
    const abrirWaMe = () => {
      try {
        window.open('https://wa.me/5562996952055?text=' + encodeURIComponent(texto), '_blank', 'noopener');
        return true;
    ...
    if (gw) {
      try {
        fetch(gw, {
          ...
          body: JSON.stringify({ para: '5562996952055', texto }),
  ...
  function unidadeAtual() {
    return (DB().meta.ler().unidadeNome || '').trim() || 'USF Jardim Cerrado IV';
  }
  function unidadeInfoAtual() {
    const m = DB().meta.ler();
    return {
      nome: (m.unidadeNome || '').trim() || 'USF Jardim Cerrado IV',
      end: (m.unidadeEndereco || '').trim() || 'Rua Pingo do Ouro; Qd 17 - Jd. Cerrado IV',
      cid: (m.unidadeCidadeCep || '').trim() || 'Goiânia/GO - CEP 74.491-510',
    };
  }
  ...
  function personalizar() {
    ...
    if (window.TEMA && window.TEMA.m) window.TEMA.m(true);
  ...
  /* ==========================================================================
     ...
     Dados SOBERANOS (js/farmacia.js, js/dossie-m.js): renderiza como
  ...
  function montarDossie() {
    ...
    if (!mount || !window.FARMACIA_CSF || !window.DOSSIE_M.) return;
    const dos = window.DOSSIE_M.;
    const farm = window.FARMACIA_CSF;
  ...
  function armarCoroas() {
    ...
    slots.forEach(slot => {
      ...
      const marcado = !!(topico && DB().draM..ehFavorito(topico.id, rid));
      slot.innerHTML =
  ...
  function coroarEsquema(regimenId) {
    ...
    const r = DB().draM..adicionarOuAtualizarProprio(topico.id, regimen.titulo, regimen.prescription || '', tipo);
  ...
  function alternarEstrelaCard(regimenId, btn) {
    ...
    if (!topico) return;
    const regimen = (topico.regimens || []).find(r => r.id === regimenId);
    const on = DB().draM..alternarFavorito(topico.id, regimenId, regimen ? regimen.titulo : '');
  ...
  function aoClicarExtra(ev) {
    ...
    else if (acao === 'sair') { DB().sessao.sair(); if (window.TEMA && window.TEMA.m) window.TEMA.m(false); location.reload(); }
