/* ============================================================================
   F2b — O BOTÃO DA DRA. M. (frente mais importante)
   ----------------------------------------------------------------------------
   O que faz: dentro de CADA doença do HUB, no TOPO do workspace, monta a seção
   "Esquemas da Dra. M.": os esquemas PRÓPRIOS dela (selo "Meu esquema")
...
window.F2 = (() => {
  ...
  // REFERÊNCIA ("Referência da Dra. M.") — NÃO é esquema imprimível do
  ...
  function cartoesReferenciaHTML() {
    if (!window.DOSSIE_MAPA || !window.DOSSIE_M.) return '';
    ...
    return temas.map(temaNome => {
      const esq = (window.DOSSIE_M..esqueminhas || []).find(e => e.tema === temaNome);
      if (!esq) return '';
      return '<aside class="f2-ref-dra" aria-label="Referência da Dra. M. — ' + esc(temaNome) + '">' +
        '<h3><span class="orq-esteto" aria-hidden="true"></span> Referência da Dra. M. <span class="f2-ref-tema">' + esc(temaNome) + '</span></h3>' +
  ...
  function viewSoapHTML() {
    ...
    return '<p class="f2-dra-sub">Nota de atendimento no padrão PEC, já ligada a esta doença.</p>' +
      '<div class="f2-form f2-soap-form">' +
        '<label for="f2SoapPac">Paciente</label>' +
        '<select id="f2SoapPac">' +
        ...
        '<div class="f2-soap">' +
          ...
          '<section class="f2-soap-bloco f2-soap-o"><h3><span class="f2-letra">O</span> Objetivo — o que você observa e mede</h3>' +
            ...
            '<div class="f2-triagem">' +
              '<div class="f2-tri-campo"><label for="f2TriPeso">Peso (kg)</label><input type="number" id="f2TriPeso" min="0" step="0.1" inputmode="decimal"></div>' +
  ...
  function ligarFormSoap() {
    ...
    // (1) toque no CID leva à AVALIAÇÃO (não mais ao Plano).
      ...
      if (acao === 'hoje') {
        ...
        try {
          ...
          if (pid === '__novo__') {
            ...
            pid = DB().patients.criar({ nome, nascimento: mount.querySelector('#f2SoapNovoNasc').value }).id;
          }
  ...
  function abrirFormulario(edicao) {
    ...
    form.addEventListener('click', ev => {
      ...
      if (btn.dataset.f2f === 'salvar') {
        ...
        try {
          if (edicao) {
            DB().draM..atualizarProprio(edicao.id, { nome, texto, tipo });
            avisar('Esquema atualizado.');
          } else {
            DB().draM..adicionarProprio(topicoAtual.id, nome, texto, tipo);
  ...
  function montarBotaoEstrela() {
    ...
    // F-I: se os cards do codex existirem, a estrela vive nos cards (m-extra);
    ...
    btn.addEventListener('click', () => {
      ...
      if (!regimen) return;
      const marcou = DB().draM..alternarFavorito(topicoAtual.id, regimen.id, regimen.titulo);
  ...
  function atualizarEstrela() {
    ...
    const on = topicoAtual && select.value && DB().draM..ehFavorito(topicoAtual.id, select.value);
  ...
  function exportarEsquemas() {
    baixarArquivo(DB().nomeArquivoDraM.(), DB().exportarDraM.());
  ...
  function importarEsquemas(input) {
    ...
    leitor.onload = () => {
      try {
        ...
        const r = DB().importarDraM.(obj); // valida ANTES de gravar
  ...
  function imprimirEsquema(id) {
    ...
    const e = DB().draM..propriosDoTopico(topicoAtual.id).find(x => x.id === id);
    if (!e) return;
    const folha =
      '<section class="f2-print-sheet">' +
        '<div class="f2-print-head"><h1>' + esc(e.nome) + '</h1>' +
        '<p>Esquema pessoal da Dra. M. · ' + esc(topicoAtual.titulo) + ' · ' + new Date().toLocaleDateString('pt-BR') + '</p></div>' +
        ...
        '<div class="f2-print-rodape"><span>Hub de Receituários UBS 2026 — módulo Dra. M.</span><span>Uso interno</span></div>' +
      '</section>';
  ...
  function aoClicar(ev) {
    ...
    if (acao === 'adicionar') abrirFormulario(null);
      ...
      const e = DB().draM..propriosDoTopico(topicoAtual.id).find(x => x.id === btn.dataset.id);
      ...
      confirmar('Apagar este esquema?',
        ...
        'Apagar', () => { DB().draM..apagarProprio(e.id); avisar('Esquema apagado.'); renderizarSecao(); });
    }
    else if (acao === 'abrir') {
      const select = $('#regimenSelect');
      if (select) {
      ...
      if (!topicoAtual) return;
      DB().draM..removerFavorito(topicoAtual.id, btn.dataset.regimen);
