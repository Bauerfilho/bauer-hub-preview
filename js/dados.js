...
window.F2DB = (() => {
  ...
  const K = {
    ...
    draM.: 'ubs2026.v1.draM.', // esquemas preferidos e próprios da Dra.
  ...
  // ==========================================================================
  // PACIENTES — [{id, nome, nascimento?, cns?, createdAt, updatedAt}]
  // ==========================================================================
  const patients = {
    listar() {
    ...
    buscar(id) {
      return patients.listar().find(p => p.id === id) || null;
    },
    ...
    procurar(texto) {
      ...
      return patients.listar().filter(p =>
        norm(p.nome).includes(t) || norm(p.cns || '').includes(t));
    },
    criar(dados) {
  ...
  const consults = {
    ...
    // Roda no boot (m-extra). Devolve quantas entradas saíram.
  ...
  // ==========================================================================
  // DRA. M. — preferências da médica (NÃO é dado de paciente).
  ...
  const draM. = {
    _ler() {
      const d = lerJSON(K.draM., null);
      return (d && Array.isArray(d.favorites) && Array.isArray(d.own))
        ? d : { favorites: [], own: [] };
    },
    _gravar(d) { gravarJSON(K.draM., d); },

    // --- Favoritos do guia (espelho ⭐) ---
    favoritosDoTopico(topicId) {
      return draM.._ler().favorites.filter(f => f.topicId === topicId);
    },
    ehFavorito(topicId, regimenId) {
      return draM.._ler().favorites.some(f => f.topicId === topicId && f.regimenId === regimenId);
    },
    ...
    alternarFavorito(topicId, regimenId, titulo) {
      ...
      if (i >= 0) { d.favorites.splice(i, 1); draM.._gravar(d); return false; }
    ...
    adicionarOuAtualizarProprio(topicId, nome, texto, tipo) {
      ...
      const e = draM..adicionarProprio(topicId, nomeLimpo, texto, tipo);
  ...
  //  (b) esquemas da Dra. M. = preferência de médica, não sensível
  // ==========================================================================
  ...
  function exportarDraM.() {
    return {
      v: VERSAO, tipo: 'ubs2026-dram',
      exportadoEm: new Date().toISOString(),
      draM.: draM.._ler(),
    };
  }
  function nomeArquivoPacientes() { return 'BACKUP-PACIENTES-SENSIVEL-' + dataHoje() + '.json'; }
  function nomeArquivoDraM.() { return 'esquemas-dra-m-' + dataHoje() + '.json'; }
  ...
  function validarDraM.(obj) {
    if (!obj || obj.tipo !== 'ubs2026-dram' || !obj.draM. ||
        !Array.isArray(obj.draM..favorites) || !Array.isArray(obj.draM..own)) {
  ...
  function importarDraM.(obj) {
    validarDraM.(obj);
    const atual = draM.._ler();
    const novo = obj.draM.;
    const porId = new Map();
    ...
    draM.._gravar({ favorites: [...favs.values()], own: [...porId.values()] });
  ...
  // ==========================================================================
  // SESSÃO (F-D) — login da Dra. M.. É IDENTIDADE E ACOLHIMENTO, não
  ...
  const sessao = {
    ...
    // Adendo 5: DUAS entradas com a mesma senha — o CRM 24644 OU o RQE 17737
    ...
    // CRM 24644 — validado no fluxo, em m-extra.js).
    trocarSenha(nova) {
  ...
  return { patients, consults, draM., meta, sessao, usageBytes, apagarTudo, norm,
           idadeTexto, idadeDetalhada, dataBR, plural, hctzAcimaDe25, rodapeDraHTML,
           exportarPacientes, exportarDraM., importarPacientes, importarDraM.,
           validarPacientes, validarDraM., nomeArquivoPacientes, nomeArquivoDraM., novoId };
})();
