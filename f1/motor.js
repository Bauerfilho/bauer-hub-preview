(function iniciarMotorF1(escopoGlobal, documentoHtml) {
  ...
  function cabecalhoInstitucional() {
    return `
      <header class="form-header" data-printable>
        <div class="unit-line">
          <strong>USF JARDIM CERRADO IV</strong>
          <span>Rua Pingo do Ouro; Qd 17 - Jd. Cerrado IV<br>Goiânia/GO - CEP 74.491-510</span>
        </div>
  ...
  function entrada(chave, rotulo, editavel, opcoes = {}) {
    ...
    if (!editavel) {
      return `<span class="mirror-value ${classe}" data-espelho="${escaparHtml(chave)}" data-tipo="${escaparHtml(tipo)}" data-printable></span>`;
    }
  ...
  function seletor(chave, rotulo, editavel, opcoes = {}) {
    if (!editavel) {
      return `<span class="mirror-value ${opcoes.classe || ''}" data-espelho="${escaparHtml(chave)}" data-tipo="select" data-printable></span>`;
    }
  ...
  function grupoRadio(chave, rotulo, opcoes, editavel) {
    if (!editavel) {
      return `<div class="radio-group mirror-radio" data-printable>
        ...
        <span class="mirror-value" data-espelho="${escaparHtml(chave)}" data-tipo="radio"></span>
      </div>`;
    }
  ...
  function corpoControleEspecial(documento, editavel) {
    ...
    return `
      <div class="special-prescription">
        ...
        <h2 class="form-title special-title" data-printable>${escaparHtml(documento.tituloFormulario)}</h2>
  ...
  function iniciar() {
    ...
    const parametros = new URLSearchParams(escopoGlobal.location.search);
    const idInicial = parametros.get('doc');
    abrirDocumento(escopoGlobal.F1Registro.obter(idInicial) ? idInicial : documentos[0].id);

    if (parametros.get('synthetic') === '1') {
      preencherSintetico();
    }
    if (parametros.get('selftest') === '1') {
      executarAutoteste();
    }
  }

  escopoGlobal.addEventListener('beforeprint', (evento) => {
    if (impressaoAutorizada || new URLSearchParams(escopoGlobal.location.search).get('proof') === '1') {
      return;
    }
    evento.preventDefault();
    documentoHtml.body.dataset.impressaoBloqueada = 'true';
  });

  escopoGlobal.addEventListener('afterprint', () => {
    impressaoAutorizada = false;
    delete documentoHtml.body.dataset.impressaoBloqueada;
  });

  escopoGlobal.F1Teste = Object.freeze({
